...
public class HeroesService : IHeroesService
{
    ...
    public async Task<HeroListModel> GetHeroListModelAsync(Guid? ownerId)
    {
        ...
        heroes = await _dbContext.Heroes
            .Include(h => h.Club)
            .ToListAsync();

        if (ownerId.HasValue) heroes = heroes.Where(h => h.Club.OwnerId == ownerId.Value).ToList();
    ...
    public async Task<HeroEditModel> CreateHeroEditModelAsync(Guid? ownerId)
    {
        ...
        if (ownerId.HasValue)
        {
            clubs = await _dbContext.Clubs.Where(c => c.OwnerId == ownerId.Value).ToListAsync();
    ...
    public async Task<HeroEditModel> GetHeroEditModelByIdAsync(Guid id, Guid? ownerId)
    {
        ...
        if (ownerId.HasValue && hero.Club.OwnerId != ownerId) return null;
        ...
        if (clubs == null) clubs = new List<Club>();
    ...
    {
        var hero = _dbContext.Heroes.FirstOrDefault(c => c.Id == id);
    ...
    {
        if (club == null) throw new ArgumentNullException("Club cannot be null.");

        foreach (var hero in club.Heroes) hero.IsCoordinator = false;

        newCoordinatorsIds.ToList().ForEach(id =>
        {
            var newCoordinator = _dbContext.Heroes.FirstOrDefault(h => h.Id == id);
    ...
    public async Task<ICollection<Hero>> GetHeroes(Guid? clubId, Guid? ownerId)
    {
        ...
        if (clubId.HasValue) heroes = heroes.Where(h => h.ClubId == clubId);

        if (ownerId.HasValue) heroes = heroes.Where(h => h.Club.OwnerId == ownerId.Value);
    ...
    {
        return await _dbContext.Heroes.FirstOrDefaultAsync(h => h.Id == id);
    }
}